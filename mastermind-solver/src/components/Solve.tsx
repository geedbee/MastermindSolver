...
export default function Solve() {
    ...
    const [currFeedback, setCurrFeedback] = useState<number[]>([]);
    ...
    //Function: Make new guess from curr guess MODIFIED
    //Input: None
    //Output: None
    function makeNewGuess() {
        let count = 0;
        for (let i = 0; i < currFeedback.length; i++){
            if (currFeedback[i] === 0){
                count++;
            }
        }
        ...
        //Minimax maintenance
        pruneList(currGuess, stringifyBW(currFeedback), knuth_codes);
    ...
    function undoFeedback(){
        setCurrFeedback(currFeedback.slice(0,currFeedback.length-1));
    }
    function updateFeedback(i: number){
        if (currFeedback.length < 4){
            setCurrFeedback(() => [...currFeedback, i]);
        }
    }
        ...
        <>
            ...
            <div className="play-board">
                ...
                {!solved && <div className="hint-box">
                    <Guess colors={COLORS} guess={bestGuess} feedbackColors={FEEDBACK_COLORS} currFeedback={[]}></Guess>
                </div>}
            </div>
            <div className="buttons">
                ...
                {FEEDBACK_COLORS.map((x: string, i: number) => <button key={i}
                                                                       title="Press me"
                                                                       style={{"backgroundColor": x}}
                ...
                <button className="enter-button" title="undo" onClick={undoGuess}>Undo Color</button>
                <button className="enter-button" title="undo" onClick={undoFeedback}>Undo Feedback</button>
